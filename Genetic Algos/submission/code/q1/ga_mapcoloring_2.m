...
load usmap; 
params = struct(...
    'psize',1000,...%m %change to 100,1000
    'offspringsize',998,...%l %change to 8,1000-2
    'n',50,...%N
    'maxiter',500,...%mtimer
    'tournament',20,...%k
    'mutationrate',0.1,...%pm
...
p=p(:,id); %SORTED P and score has been calculated for first run
%%
for run=1:mtimer
...
[ii,jj]=size(p);
 ...
 for i=1:jj
     for j=1:ii
     if sum(p(find(A(:,j)==1),i)==p(j,i))>0 % check how close the vector is to the final value
         sc(1,i)=sc(1,i)+1;
     end
 end
 end
...
%% MUTATION of the child based on the rate pm
...
%% SURVIVE usling elitism choose the best
function [p,sc]=ss(par,chil,A)
a=[par,chil];
sc1=fit(a,A);
[sc1,id]=sort(sc1,'ascend');
s=a(:,id);
s(:,1001:end)=[];%change according to number of parent,child
p=s;
sc1(:,1001:end)=[];%change according to number of parent,child
sc=sc1;
end
